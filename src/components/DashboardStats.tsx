...
import { Package, Truck, Clock, CheckCircle, AlertTriangle, TrendingUp, MapPin, ArrowRight, Sparkles, Activity } from 'lucide-react';
...
const StatCard: React.FC<{
  ...
  transition: {
    ...
    <div className={cn("absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300", accent === 'primary' && "bg-gradient-to-br from-primary/5 to-transparent", accent === 'success' && "bg-gradient-to-br from-success/5 to-transparent", accent === 'destructive' && "bg-gradient-to-br from-destructive/5 to-transparent")} />
    
    <div className="relative z-10">
      <div className="flex items-start justify-between mb-4">
        <div className={cn("p-2.5 rounded-xl transition-colors", accent === 'primary' && "bg-primary/10 group-hover:bg-primary/20", accent === 'success' && "bg-success/10 group-hover:bg-success/20", accent === 'destructive' && "bg-destructive/10 group-hover:bg-destructive/20")}>
          {icon}
        </div>
        {trend && <div className={cn('flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full', trend === 'up' && 'text-success bg-success/10', trend === 'down' && 'text-destructive bg-destructive/10', trend === 'neutral' && 'text-muted-foreground bg-muted/30')}>
...
const DashboardStatsComponent: React.FC<DashboardStatsProps> = ({
  ...
  return <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:block">
              ...
              <div className="flex h-2 w-48 rounded-full overflow-hidden bg-muted/30">
                <motion.div className="bg-primary h-full" initial={{
                width: 0
              }} animate={{
                ...
                <motion.div className="bg-success h-full" initial={{
                width: 0
              }} animate={{
                ...
                <motion.div className="bg-destructive h-full" initial={{
                width: 0
              }} animate={{
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={<Package className="w-5 h-5 text-primary" />} label="Total Shipments" value={stats.totalShipments} delay={0} accent="primary" />
        <StatCard icon={<Truck className="w-5 h-5 text-primary" />} label="In Transit" value={stats.inTransit} subValue="+12%" trend="up" delay={0.1} accent="primary" />
        <StatCard icon={<CheckCircle className="w-5 h-5 text-success" />} label="Delivered" value={stats.delivered} subValue={`${stats.onTimeRate}% on-time`} trend="up" delay={0.2} accent="success" />
        <StatCard icon={<AlertTriangle className="w-5 h-5 text-destructive" />} label="Delayed" value={stats.delayed} subValue="-8%" trend="down" delay={0.3} accent="destructive" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        delay: 0.4
          ...
          <div className="relative z-10">
            ...
            <div className="h-2.5 bg-muted/30 rounded-full overflow-hidden">
              <motion.div className="h-full bg-gradient-to-r from-primary via-primary to-success rounded-full" initial={{
              width: 0
            }} animate={{
              width: `${Math.min(stats.averageDeliveryTime / 48 * 100, 100)}%`
            }} transition={{
              delay: 0.6,
              duration: 0.8
            }} />
        ...
        delay: 0.5
          ...
          <div className="relative z-10">
            ...
            <div className="h-2.5 bg-muted/30 rounded-full overflow-hidden">
              <motion.div className="h-full bg-gradient-to-r from-success via-success to-primary rounded-full" initial={{
              width: 0
            }} animate={{
              width: `${stats.onTimeRate}%`
            }} transition={{
              delay: 0.7,
              duration: 0.8
            }} />
      ...
      delay: 0.6
        ...
        <div className="space-y-2">
          {recentShipments.slice(0, 5).map((shipment, index) => <motion.button key={shipment.id} initial={{
          opacity: 0,
          x: -20
        }} animate={{
          opacity: 1,
        ...
        }} onClick={() => onViewShipment(shipment)} className="w-full p-4 rounded-xl bg-muted/10 hover:bg-muted/30 border border-border/30 hover:border-primary/30 transition-all duration-200 text-left group">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className={cn("p-2.5 rounded-xl transition-colors", shipment.status === 'delivered' && "bg-success/10 group-hover:bg-success/20", shipment.status === 'in_transit' && "bg-primary/10 group-hover:bg-primary/20", shipment.status === 'delayed' && "bg-destructive/10 group-hover:bg-destructive/20", !['delivered', 'in_transit', 'delayed'].includes(shipment.status) && "bg-muted/20 group-hover:bg-muted/30")}>
                ...
                <div className="text-right flex flex-col items-end gap-1.5">
                  <div className={cn('text-xs font-medium px-2.5 py-1 rounded-full text-center min-w-[80px]', shipment.status === 'delivered' && 'bg-success/20 text-success', shipment.status === 'in_transit' && 'bg-primary/20 text-primary', shipment.status === 'delayed' && 'bg-destructive/20 text-destructive', !['delivered', 'in_transit', 'delayed'].includes(shipment.status) && 'bg-muted/50 text-muted-foreground')}>
