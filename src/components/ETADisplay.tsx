...
const ETADisplay: React.FC<ETADisplayProps> = ({
  ...
  // Fetch traffic + weather on mount and refresh traffic every 60s
  useEffect(() => {
    fetchTrafficData();
    fetchWeatherData();
  ...
  // Fake loading bars for “live” status
  useEffect(() => {
  ...
  // Get specific factor data from ETA model
  ...
  const getTrafficImpactLevel = () => {
    if (!liveTrafficData) return { level: "moderate", color: "amber" as const };

    switch (liveTrafficData.status) {
      case "green":
        return { level: "low" as const, color: "teal" as const };
      case "yellow":
        return { level: "moderate" as const, color: "amber" as const };
      case "red":
        return { level: "high" as const, color: "red" as const };
      default:
        return { level: "moderate" as const, color: "amber" as const };
    }
  };
  ...
  const getTrafficDescription = () => {
    ...
    switch (liveTrafficData.status) {
      case "green":
        return `Light traffic on route to ${destinationCity}`;
      case "yellow":
        return `Moderate congestion approaching ${destinationCity}`;
      case "red":
        return `Heavy traffic / delays near ${destinationCity}`;
      default:
        return `Traffic conditions to ${destinationCity}`;
    }
  };

  // This is the key fix: derive EFFECTIVE delay in hours from TomTom status + ETA factor
  const getTrafficDelayHours = () => {
    if (!trafficFactor) return 0;

    const base = trafficFactor.adjustment;

    // No live data → keep model’s number
    if (!liveTrafficData) return base;

    switch (liveTrafficData.status) {
      case "green":
        // TomTom says route is clear → treat as no delay
        return 0;
      case "yellow":
        // Moderate congestion → at least some impact
        return base > 0 ? base : 0.5;
      case "red":
        // Heavy traffic → enforce a meaningful delay
  ...
  const trafficDelayHours = getTrafficDelayHours();

  // ---------- Weather (OpenWeather) ----------

  const getWeatherImpactLevel = () => {
    if (!liveWeatherData) return { level: "moderate", color: "amber" as const };
  ...
  const weatherDelayHours = liveWeatherData
    ? liveWeatherData.routeImpact.delayFactor * (weatherFactor?.adjustment || 0.5)
    : weatherFactor?.adjustment || 0.5;

  // ---------- Effective totals (used for Total Impact) ----------

  const effectiveCarrierAdj = carrierFactor?.adjustment ?? 0;
  const effectiveTrafficAdj = trafficFactor ? trafficDelayHours : 0;
  const effectiveDayAdj = dayFactor?.adjustment ?? 0;
  const effectiveWeatherAdj = weatherFactor ? weatherDelayHours : 0;

  const totalAdjustment = effectiveCarrierAdj + effectiveTrafficAdj + effectiveDayAdj + effectiveWeatherAdj;
            ...
            >
              ...
              <div className="relative flex items-center gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  >
                    {carrierFactor.adjustment === 0
                      ? "No measurable impact on transit time"
                      : carrierFactor.description}
                  </p>
                </div>
                <motion.div
                  className={cn(
            ...
            >
              ...
              <div className="relative flex items-center gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  >
                    {dayFactor.adjustment === 0 ? "Conditions stable" : dayFactor.description}
                  </p>
                </div>
                <motion.div
                  className={cn(
              ...
              <div className="relative flex items-center gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {liveWeatherData && liveWeatherData.routeImpact.warnings.length > 1 && (
                    ...
                    >
                      {liveWeatherData.routeImpact.warnings[1]}
                    </p>
                  )}
                </div>
                <motion.div
